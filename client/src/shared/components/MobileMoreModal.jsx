...
export default function MobileMoreModal() {
  const { logout } = useAuth();

  return (
    <motion.dialog
      open
      initial={{ opacity: 0, scale: 0, y: 400, x: 200 }}
    ...
    >
      ...
      <ul className="grid grid-cols-2 gap-4 mt-8">
        ...
        <li>
          <button
            onClick={logout}
            className="flex flex-col items-center justify-center p-4 h-24 bg-white/80 text-black backdrop-blur-md border border-white/20 rounded-lg transition-all duration-300 transform shadow-lg w-full"
          >
            <FontAwesomeIcon icon={faSignOutAlt} className=" text-2xl mb-2" />
